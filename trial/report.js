document.addEventListener("DOMContentLoaded", function () {
    ...
    if (wasteData.length === 0) {
        recyclableTable.innerHTML = "<tr><td colspan='5'>No recyclable waste reported.</td></tr>";
        disposableTable.innerHTML = "<tr><td colspan='5'>No disposable waste reported.</td></tr>";
    } else {
        wasteData.forEach(item => {
            let row = `<tr>
                <td>${item.name.replace("-", " ")}</td>
                <td>${item.type}</td>
                <td>${item.use}</td> <!-- Added Use Category Column -->
                <td>${item.quantity}</td>
